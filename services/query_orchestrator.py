...
class QueryOrchestrator:
    ...
    def analyze_user_intent(self, user_query: str, planner_model: Optional[str] = None) -> Dict[str, Any]:
        ...
        """
        print(f"[QueryOrchestrator] Stage 1: Analyzing user intent...")
...
Intent Analysis:"""
        ...
        try:
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                print(f"[QueryOrchestrator] Intent extracted: task_type={intent.get('task_type')}, time_scope={intent.get('data_requirements', {}).get('time_scope')}")
                return intent
            else:
                raise ValueError("No JSON found in response")
        except (json.JSONDecodeError, ValueError) as e:
            print(f"[QueryOrchestrator] Failed to parse intent: {e}")
            print(f"[QueryOrchestrator] Response was: {response}")
            return {
                "task_type": "general_info",
    ...
    def _get_client_for_model(self, model_name: Optional[str]):
        ...
        print(f"[QueryOrchestrator] _get_client_for_model called with: '{model_name}'")
        ...
        if not model_name:
            ...
            if self.openrouter_client and self.openrouter_client.api_key:
                print(f"[QueryOrchestrator] No model specified, using OpenRouter default")
                return self.openrouter_client, self.openrouter_client.general_model
            print(f"[QueryOrchestrator] No model specified, using Ollama default")
            return self.ollama_client, self.ollama_client.general_model
        
        if model_name.startswith("Ollama:"):
            stripped = model_name.replace("Ollama: ", "")
            print(f"[QueryOrchestrator] Using Ollama with model: '{stripped}'")
            ...
            print(f"[QueryOrchestrator] Using OpenRouter with model: '{stripped}'")
            return self.openrouter_client, stripped
        else:
            # Default to Ollama for backward compatibility
            print(f"[QueryOrchestrator] No prefix detected, defaulting to Ollama with model: '{model_name}'")
    ...
    def create_execution_plan(self, user_query: str, planner_model: Optional[str] = None, intent: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        """
        print(f"[QueryOrchestrator] create_execution_plan called with planner_model: '{planner_model}'")
        
        # Stage 1: Analyze user intent if not provided
        if not intent:
            print(f"[QueryOrchestrator] No intent provided, running Stage 1 first...")
            intent = self.analyze_user_intent(user_query, planner_model)
        
        print(f"[QueryOrchestrator] Stage 2: Creating technical execution plan from intent...")
...
Technical Execution Plan:"""
    ...
    def execute_plan(self, plan: Dict[str, Any], user_parameter_selections: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        ...
        print(f"[QueryOrchestrator] All configured servers: {list(all_servers.keys())}")
        print(f"[QueryOrchestrator] Required servers: {required_servers}")
        ...
        for server_name in required_servers:
            if server_name in all_servers:
                ...
                if server_data:
                    ...
                    if api_errors:
                        print(f"[QueryOrchestrator] Server '{server_name}' had {len(api_errors)} API errors but {len(successful_tools)} successful tools")
    ...
    def _query_mcp_server(self, server_name: str, data_points: List[str], user_parameter_selections: Optional[Dict[str, str]] = None, user_query: str = "") -> Optional[Dict[str, Any]]:
        ...
        if server_name == 'quantum-gw-cli' and 'quantum-management' in servers:
            ...
            print(f"[QueryOrchestrator] Copied management credentials to quantum-gw-cli: {list(mgmt_env.keys())}")
        ...
        try:
            ...
            print(f"[QueryOrchestrator] Error querying MCP server {server_name}: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _reduce_context_intelligently(self, data_collected: Dict[str, Any], max_tokens: int) -> Dict[str, Any]:
        ...
        while attempts < max_attempts:
            ...
            if current_size <= max_chars:
                print(f"[QueryOrchestrator] Context fits within budget after {attempts} reduction passes")
                break
            
            print(f"[QueryOrchestrator] Reduction pass {attempts+1}: {current_size} chars, target {max_chars}")
        ...
        if len(final_json) > max_chars:
            print(f"[QueryOrchestrator] EMERGENCY: Still {len(final_json)} chars after {attempts} passes, applying final truncation")
            ...
            print(f"[QueryOrchestrator] Emergency truncation complete: {len(json.dumps(reduced_data)):,} chars")
    ...
    def _sequential_chunked_analysis(
        ...
        """
        print(f"[QueryOrchestrator] Starting sequential chunked analysis...")
        ...
        print(f"[QueryOrchestrator] Split context into {total_chunks} chunks (~{chunk_size_tokens:,} tokens each)")
        ...
        if not hasattr(client, 'generate_response_with_history'):
            print(f"[QueryOrchestrator] Client doesn't support conversation history - using single-request chunking")
...
Please acknowledge receipt. Store this data in your memory. DO NOT analyze yet - wait for all {total_chunks} chunks."""
            ...
            print(f"[QueryOrchestrator] Sending chunk {chunk_num}/{total_chunks} to LLM (conversation turn {len(messages)//2})...")
            ...
            if is_final:
                print(f"[QueryOrchestrator] ✓ Sequential conversation complete - analyzed all {total_chunks} chunks")
                print(f"[QueryOrchestrator] Final conversation: {len(messages)} messages, {len(messages)//2} turns")
                return response
            else:
                print(f"[QueryOrchestrator] ✓ Chunk {chunk_num} stored in conversation (response: {len(response)} chars)")
    ...
    def _single_request_chunked_analysis(self, chunks, total_chunks, user_query, analysis_prompt, client, model_name):
        ...
        print(f"[QueryOrchestrator] Sending {total_chunks} chunks in single request (~{len(accumulated_context)//4:,} tokens)")
    ...
    def analyze_with_model(self, plan: Dict[str, Any], execution_results: Dict[str, Any], security_model: Optional[str] = None) -> Tuple[str, str]:
        ...
        print(f"[QueryOrchestrator] analyze_with_model using: '{final_model}'")
...
Be concise and focus on what's actually in the data. If the data doesn't fully answer the question, clearly state what's available and what additional data might be needed."""
        ...
        if isinstance(final_model, str) and (":" in final_model):
            ...
            print(f"[QueryOrchestrator] Client type: {client.__class__.__name__}, Model: {model_name}")
        else:
            client = self.ollama_client
            model_name = final_model
            print(f"[QueryOrchestrator] Using Ollama client (no prefix detected), Model: {model_name}")
        ...
        print(f"[QueryOrchestrator] Analysis type: {'log/threat' if is_log_analysis else 'standard'}")
        print(f"[QueryOrchestrator] Model context limit: {model_context_limit:,} tokens (max input: {max_input_tokens:,})")
        print(f"[QueryOrchestrator] Current data size: ~{estimated_tokens:,} tokens")
...
"""
                ...
                if len(context) > max_chars:
                    ...
                    print(f"[QueryOrchestrator] Context truncated from ~{estimated_tokens:,} to ~{len(context) // 4:,} tokens")
            else:
...
- Check the console logs for detailed error information"""
        ...
        if user_warning:
            ...
            print(f"[QueryOrchestrator] User warning prepended to analysis response")
